...
export default function ParticipatedEvents() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Only participated events
      // Exclude events created by the committee member himself
      // Show only Present attendance

      const participated = regsRes.data.filter((reg) => {
        if (!reg.eventId) return false

        const createdById =
          reg.eventId.createdBy?._id || reg.eventId.createdBy
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-500 mt-1">
          Events where you have participated successfully
        </p>
      </div>
            ...
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="text-center px-6 py-3 text-sm font-semibold text-slate-900">
                    Status
                  </th>
